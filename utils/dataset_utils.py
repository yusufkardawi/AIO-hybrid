...
class PromptTrainDataset(Dataset):
    ...
    def _init_deblur_ids(self):
        temp_ids = []
        deblur = self.args.data_file_dir + "blurry/blurTrain.txt"
        temp_ids += [self.args.deblur_dir + id_.strip() for id_ in open(deblur)]
        ...
        print("Total Deblur Ids : {}".format(self.num_deblur))
    ...
    def _get_gt_name(self, rainy_name):
        dir_name = rainy_name.split("rainy")[0] + 'gt/'
        name = rainy_name.split('/')[-1]
    ...
    def _get_nonhazy_name(self, hazy_name):
        ...
        nonhazy_name = dir_name + name + '.jpg'
        return nonhazy_name
    
    def _get_nonblur_name(self, blur_name):
        dir_name = blur_name.split("blurry")[0] + 'sharp/'
        name = blur_name.split('/')[-1]
        nonblur_name = dir_name + name
    ...
    def _merge_ids(self):
        self.sample_ids = []
        if "denoise_15" in self.de_type or "denoise_25" in self.de_type or "denoise_50" in self.de_type:
            if hasattr(self, 's15_ids'): self.sample_ids += self.s15_ids
            if hasattr(self, 's25_ids'): self.sample_ids += self.s25_ids
            if hasattr(self, 's50_ids'): self.sample_ids += self.s50_ids
        if "derain" in self.de_type:
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            if de_id == 3:
                ...
                # Deblurring
                degrad_img = crop_img(np.array(Image.open(sample["clean_id"]).convert('RGB')), base=16)
                #clean_name = sample["clean_id"].replace("blur", "blur")
                clean_name = self._get_nonblur_name(sample["clean_id"])
                clean_img = crop_img(np.array(Image.open(clean_name).convert('RGB')), base=16)
...
class DerainDehazeDeblurDataset(Dataset):
    def __init__(self, args, task="dehaze", addnoise=False, sigma=None):
        super(DerainDehazeDeblurDataset, self).__init__()
        self.ids = []
        self.task_idx = 0
        self.args = args

        self.task_dict = {'derain': 0, 'dehaze': 1, 'deblur': 2}
        self.toTensor = ToTensor()
    ...
    def __getitem__(self, idx):
        ...
        # Ensure the .png extension is added to clean_path
...
class RealworldTestDataset(Dataset):
    def __init__(self, opt):
        super(RealworldTestDataset, self).__init__()
        self.opt = opt
        self.image_dir = opt.realworld_path
        
        # Filter and sort image list
    ...
    def __getitem__(self, idx):
        ...
        # Return image name without extension and tensor
        return [os.path.splitext(image_name)[0]], image_tensor
...
class TestSpecificDataset(Dataset):
    ...
    def _init_clean_ids(self, root):
        extensions = ['jpg', 'JPG', 'png', 'PNG', 'jpeg', 'JPEG', 'bmp', 'BMP', 'tif']
