...
class CNNBlock(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0., LayerNorm_type='WithBias'):
        super().__init__()
        ...
        self.norm1 = LayerNorm(c, LayerNorm_type)
        self.norm2 = LayerNorm(c, LayerNorm_type)
...
class PromptIR(nn.Module):
    def __init__(
        ...
        if self.decoder:
            self.prompt1 = PromptGenBlock(
                prompt_dim=64, prompt_len=6, prompt_size=64, lin_dim=96
            )
            self.prompt2 = PromptGenBlock(
                prompt_dim=128, prompt_len=6, prompt_size=32, lin_dim=192
            )
            self.prompt3 = PromptGenBlock(
                prompt_dim=320, prompt_len=6, prompt_size=16, lin_dim=384
            )
            #self.deblurring_prompt = PromptGenBlock(
                #prompt_dim=128, prompt_len=6, prompt_size=32, lin_dim=192
            #)
        ...
        self.encoder_level1 = nn.Sequential(
            *[CNNBlock(dim, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])]
        )
        ...
        self.encoder_level2 = nn.Sequential(
            *[CNNBlock(int(dim * 2 ** 1), LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])]
        )
        ...
        self.encoder_level3 = nn.Sequential(
            *[CNNBlock(int(dim * 2 ** 2), LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])]
        )

        self.down3_4 = Downsample(int(dim * 2 ** 2))  ## From Level 3 to Level 4

    
        # Latent space (Level 4) - Diganti dengan CNNBlock
        self.latent = nn.Sequential(
            *[
                CNNBlock(int(dim * 2 ** 3), LayerNorm_type=LayerNorm_type) for i in range(num_blocks[3])
            ]
        )
        ...
        self.decoder_level1 = nn.Sequential(
            *[CNNBlock(int(dim * 2 ** 1), LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])]
        )
    ...
    def forward(self, inp_img, noise_emb=None):
        ...
        # Apply the deblurring prompt in the latent space
        #deblurred_prompt = self.deblurring_prompt(latent)
        #latent = latent + deblurred_prompt
        ...
        #self.prompt1_output = inp_dec_level3 
        #self.prompt2_output = inp_dec_level2
        #self.prompt3_output = inp_dec_level1

        return out_dec_level1
        #return out_dec_level1, self.prompt1_output, self.prompt2_output, self.prompt3_output
